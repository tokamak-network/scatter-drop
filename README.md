...
모든 캠페인은 **zk-X509 신원검증**으로 게이팅된다 — 캠페인이 지정한 IdentityRegistry에 등록된 신원을
거쳐야 받을 수 있다. 그 신원은 **국가 PKI(NPKI·정부 eID, 법적·실명)일 수도, 자체 발급 CA(기업·조직)일 수도** 있다.

> 포지셔닝: "누구나 에어드랍"이 아니라 **"신원검증 기반 컴플라이언트 토큰 분배"** — 국가 PKI까지 지원해
> 실명·법적 분배가 필요한 시장(RWA·규제 토큰)을 포함.
