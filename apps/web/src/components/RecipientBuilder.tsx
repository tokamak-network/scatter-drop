...
/**
 * Two-step recipient list builder — aggregate recipients (Dune/staking/CSV),
 * then decide amounts (equal / pro-rata / √) — shared by the /tools page and
 * the campaign wizard. The final list is handed to `onUse` as an
 * `address,amount` CSV (human token units).
 *
 * `fixedToken` pins the distribution token (the wizard already picked one, so
 * its picker is hidden); omit it for the standalone /tools flow, which lets
 * the user choose from the allow-list.
 */
export function RecipientBuilder({
  ...
  fixedToken?: { address: string; decimals: number; symbol?: string };
  onUse: (csv: string) => void;
  useLabel?: string;
}) {
  ...
  // With a fixedToken the wizard owns the choice, so this local state is unused
  // (the picker is hidden); otherwise it's the standalone token select.
  const [tokenInput, setTokenInput] = useState("");
  const token = fixedToken?.address ?? tokenInput;
  ...
  const symbol = fixedToken
    ? fixedToken.symbol
    : listedSymbol ?? (typeof symData === "string" && symData ? symData : undefined);
  ...
  const dec = fixedToken ? fixedToken.decimals : tokenOk && decData != null ? Number(decData) : 18;
  const unit = symbol ?? "tokens";
  // Whether the token+decimals are ready to scale amounts (always so when the
  // wizard fixed them).
  const decReady = fixedToken ? true : tokenOk && decData != null;
  ...
  // being read) both would be guesses on the silent 18-dp fallback. (decReady
  // is derived above; the wizard's fixedToken makes it always true.)
  ...
  // wizard's CSV box expect.
  const handleUse = () => onUse(buildCsv(false));
  ...
  return (
    <div className="space-y-8">
      ...
      {step === 2 && (
        <div className="space-y-4">
          {/* Distribution token — hidden when the wizard already fixed it. */}
          {!fixedToken && (
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              className={CTA_CLS}
            >
              {useLabel} <ArrowRight className="w-4 h-4" />
            </button>
          </div>
