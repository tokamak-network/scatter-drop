...
6. **고객 신원 게이트(zk-X509 CA 레지스트리)는 운영자 선택 — 캠페인마다 켜거나 끔.** (아래 §0)

---

## 0. zk-X509 신원 게이트 — 2종 (운영자 게이트=어드민 정책 / 고객 게이트=운영자 선택)
...
### (2) 고객 게이트 — 운영자 선택 (켜거나 끔, 캠페인마다)
- 캠페인 생성 시 **고객용 IdentityRegistry 주소 지정 = 게이트 ON**, 미지정 = **OFF(open claim)**.
- **게이트 ON이면:** 받는 지갑은 반드시 그 레지스트리에서 사전 신원검증된 지갑. 미검증 수령 불가.
- **게이트 OFF면:** 신원검증 없이 머클 proof만으로 claim.
- **공통:** self-claim만 허용(`msg.sender`=수령자). 대납·타주소 수령 불가(게이트 무관).
...
─ 신원 게이트(이 캠페인이 ON일 때만) ─  지정된 zk-X509 CA Registry 검증
  · identityRegistry == 0 이면 게이트 OFF → 이 섹션 숨김(신원검증 없이 claim)
  · ON이면: verifiedUntil(claimer) >= now ?  → 미검증이면 "신원 인증 필요" 안내
...
Step 1  기본 정보   이름·설명·로고·배포 토큰·**시작시간·마감시간**
                  ...
                  · 배포 토큰: **주소 직접입력 란만**(picker·"+토큰추가" 버튼 없음). 입력 주소를 isAllowed 검증.
                    미허가 주소 시 "미허가 토큰 — 어드민에 등록 요청" 안내+차단. 운영자 토큰추가 불가, 지원토큰은 어드민 관리 (§8.7 어드민 큐레이션)
...
Tokens            /admin/tokens     ★ 에어드랍 토큰 화이트리스트 (§8.7, 어드민 큐레이션 — 어드민 전용)
 ├ 토큰 허가          setAllowedToken(token,true) — 플랫폼 적합성 큐레이션 후 허가. 주소 입력 + 메모(심볼·출처)
 ├ 허가 목록          ALLOWED 토큰 목록(심볼·주소·메모)
 └ 허가 해제          setAllowedToken(token,false) — 사칭·부적절 토큰 제거
 (운영자 셀프 등록 없음 — 어드민이 추린 established 토큰만)
