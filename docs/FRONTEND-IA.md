...
Step 1  기본 정보   이름·설명·로고·배포 토큰·**시작시간·마감시간**
                  ...
                  · 배포 토큰: **주소 직접입력 란만**(picker·"+토큰추가" 버튼 없음). 입력 주소를 isAllowed 검증.
                    미허가 주소 시 "미허가 토큰 — 어드민에 등록 요청" 안내+차단. 운영자 토큰추가 불가, 지원토큰은 어드민 관리 (§8.7 증권성)
...
Tokens            /admin/tokens     ★ 에어드랍 토큰 화이트리스트 (§8.7, 증권성 규제 — 어드민 전용)
 ├ 토큰 허가          setAllowedToken(token,true) — 증권성 심사 후 허가. 주소 입력 + 컴플라이언스 근거 메모(증권 등록번호 등)
 ├ 허가 목록          ALLOWED 토큰 목록(심볼·주소·등록근거)
 └ 허가 해제          setAllowedToken(token,false) — 문제 토큰 제거
 (운영자 셀프 등록 없음 — 무허가 증권 배포 차단)
