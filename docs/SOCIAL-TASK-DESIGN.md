...
| 태스크 | 검증 가능? | 비용 | 방법 |
|---|---|---|---|
| Discord 서버 가입 | ✅ 확실 | 무료 | OAuth2 `identify`(계정 바인딩만) + **플랫폼 봇** `GET /guilds/{id}/members/{uid}` — `guilds` 스코프(전체 서버 목록)는 프라이버시 침습적이라 요구하지 않음 |
| Discord 역할 보유 | ✅ 확실 | 무료 | 동일 봇 member 객체(roles, `joined_at`) — 운영자가 봇을 서버에 설치하는 것이 온보딩 요건 |
...
SOC-2': Discord — OAuth(identify) 바인딩 + 봇 멤버 조회 JOIN/ROLE 검증기 + WalletSocial   ★첫 수직 슬라이스
...
## 9. 확정 결정 (2026-07-07, Task #1에서 반영)

1. **X 모듈은 v1 제외** — 종량 과금($0.005–0.01/건) + 가격 변동성 리스크. v1은
   Discord/TG/GitHub/온체인만으로 출시하고, X는 수요 확인 후 별도 결정 (SOC-7'은 보류).
2. **amountMode는 v1 균등 분배(`equal`)만** — 태스크별 가중치(fixed-per-task)는 후속.
   API는 `equal` 외 값을 명시적으로 거부한다 (조용한 강등 금지 원칙과 동일).
3. **수령자 퀘스트 페이지는 별도 `/q/[id]` 라우트** — 캠페인 상세(`/c/[id]`)에서
   링크로 연결. 퀘스트는 createDrop 이전의 명단 생산 단계라 캠페인(드랍 주소) 상세에
   묶지 않는다.
