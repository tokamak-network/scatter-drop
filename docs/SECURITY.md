...
| M-2 | Medium | Fee bypass (DropFactory) | The ETH fee path (`feeToken == address(0)`) lacked the `FeeNotConfigured` guard the ERC20 path has, so if `feeOf[ETH][type]` was unset (`0`) an operator could pass `feeToken = ETH`, `msg.value = 0` and create a campaign **for free**, bypassing the configured ERC20 price for that airdrop type. | **Fixed** — the ETH branch now reverts `FeeNotConfigured` when the tier is unpriced, symmetric with ERC20: an unpriced tier is *not* a free tier. Found in the full-stack security audit; fixed in the W19/ETH-fee-guard PR. |
